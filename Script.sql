...
select employee_id , first_name , email, phone_number, salary from employees ;

select distinct job_id from employees m;

select distinct manager_id from employees m

select distinct salary from employees m

select employee_id , first_name , last_name, manager_id  from employees where manager_id =	108

select employee_id , first_name , last_name from employees where manager_id = '108' and hire_date = '1987-06-28'

select employee_id , first_name , last_name from employees where hire_date between '1987-06-20' and '1987-06-28'

select employee_id , first_name , last_name from employees where job_id in ('AD_VP','IT_PROG')

select * from job_history b where department_id is null
...
insert into users values (1, 'dwi saputra', '1997-06-07',1455874134,'Pelanggan Premium');
insert into users values (2, 'agung', '1997-09-01',46434349498798,'Pelanggan Premium');

select * from users


select manager_id , count(employee_id) as num_employee from employees e group by manager_id order by num_employee desc

select employee_id , commission_pct  * salary as salary_commision  from employees where commission_pct * salary > 0;

select e.employee_id , e.first_name , e.last_name , e.department_id , d.department_id  
	from employees e 
	JOIN  departments d 
...
select manager_id , SUM(salary) as total_salary, avg(commission_pct) as commission_pct from employees group by manager_id 


select manager_id , total_salary * 	commission_pct as commision 
from (
	select 
	manager_id , 
